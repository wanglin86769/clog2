<template>
    <div>
        ...
        <div class="register">
            ...
            <p><input type="password" v-model="account.currentPassword" placeholder="当前密码"></p>
            <hr>
            ...
            <hr>
            <p class="submit"><input type="button" value="修改" @click="updatePassword"></p>
        </div>
    ...
    methods: {
        updatePassword() {
            ...
            if(!this.account.currentPassword) {
                this.$toast.add({ severity: 'error', summary: '操作失败', detail: '当前密码不能为空！' });
                return;
            }
            ...
            .then(() => {
                this.$toast.add({severity:'success', summary: '操作完成', detail:'用户密码已更新'});
                this.reset();
            }).catch(error => {
              if(error.response) {
